...
insert into product values (16,'하의','바지',1,1,'바지입니다','1990-11-11',1);insert into product values (1,1,'검은색',1,1,'티셔츠입니다','1990-11-11',1);
...
CREATE TABLE userOrder_detail (
    order_detail_number   NUMBER(*, 0),
    order_number          VARCHAR2(30 BYTE),
    product_number        NUMBER(*, 0),
    product_count         NUMBER(*, 0),
    product_price         NUMBER(*, 0),
    order_detail_status   VARCHAR2(20 BYTE),
    constraint order_number_fk1 foreign key(order_number) references user_order(order_number) ON DELETE CASCADE,
    constraint product_number_fk2 foreign key(product_number) references product(product_number) ON DELETE CASCADE
);
insert into userOrder_detail values (1,'1',8,1,8,'입금 확인');
insert into userOrder_detail values (2,'2',8,1,8,'입금 확인');

select * from userOrder_detail;
select * from userOrder_detail;
